...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img):
        ...
        Args:
            bbox_xywh (NDArray[float64]): 目标xywh
            confidences (NDArray[float64]): 目标置信度
            ori_img (Mat): 目标图像区域
    ...
    def update_obb(self, bbox_xyxyxyxy, confidences, ori_img, xyxy):
        """更新当前追踪 OBB 目标

        Args:
            bbox_xywh (NDArray[float64]): 目标xywhr
            confidences (NDArray[float64]): 目标置信度
            ori_img (Mat): 目标图像区域
            xyxy (NDArray[float64]): 旋转目标的最小外接矩形

        Returns:
            NDArray|list: 目标追踪信息
        """
        self.height, self.width = ori_img.shape[:2]
        features = self._get_features_obb(xyxy, ori_img)
        # 使用 xyxy 来获取 tlwh 格式
        bbox_tlwh = np.array([self._xyxy_to_tlwh(box) for box in xyxy])
        detections = [
            Detection(bbox_tlwh[i], conf, features[i])
            for i, conf in enumerate(confidences)
            if conf > self.min_confidence
        ...
        for i, detection in enumerate(detections):
            detection.original_obb = bbox_xyxyxyxy[i]
        ...
        for track in self.tracker.tracks:
            ...
            box = track.to_tlwh()
            x, y, w, h = box
            if hasattr(track, 'last_detection') and hasattr(track.last_detection, 'original_obb'):
                xyxyxyxy = track.last_detection.original_obb
            elif hasattr(track, 'original_obb'):
                xyxyxyxy = track.original_obb
            else:
                xyxyxyxy = np.array([x, y, x+w, y, x+w, y+h, x, y+h])

            track_id = track.track_id
            # 确保 xyxyxyxy 是一维数组
            xyxyxyxy = xyxyxyxy.flatten()
            outputs.append(np.concatenate([xyxyxyxy, [track_id]]))

        if len(outputs) > 0:
            outputs = np.stack(outputs, axis=0)
        return outputs

    def _get_features_obb(self, xyxy, ori_img):
        im_crops = []
        for obb_xyxy in xyxy:
            x1, y1, x2, y2 = obb_xyxy

            # 确保坐标在图像范围内，并转换为整数
            x1 = max(0, int(x1))
            y1 = max(0, int(y1))
            x2 = min(int(x2), ori_img.shape[1])
            y2 = min(int(y2), ori_img.shape[0])
